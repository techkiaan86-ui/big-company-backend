...
async function testEndpoint(name: string, url: string, method: 'GET' | 'POST' = 'GET', body?: any): Promise<void> {
    try {
...
async function main() {
    ...
    // Swagger UI
    await testEndpoint('Swagger UI (Server Alive?)', `${STRONPOWER_BASE}/swagger/ui/index`);

    // Try to get the Swagger JSON spec to discover all endpoints
    await testEndpoint('Swagger JSON Spec', `${STRONPOWER_BASE}/swagger/v1/swagger.json`);
    await testEndpoint('Swagger JSON Spec (alt)', `${STRONPOWER_BASE}/api/swagger.json`);
    await testEndpoint('API Root', `${STRONPOWER_BASE}/api`);

    // Common Stronpower API endpoint patterns
    await testEndpoint('User/Auth Login', `${STRONPOWER_BASE}/api/User/login`, 'POST', {
        username: 'test', password: 'test'
    });
    await testEndpoint('User/Token', `${STRONPOWER_BASE}/api/User/token`, 'POST', {
        username: 'test', password: 'test'
    });
    await testEndpoint('Recharge Endpoint', `${STRONPOWER_BASE}/api/Order/recharge`, 'POST', {
        meter_no: 'TEST001', recharge_amount: 100
    });
    await testEndpoint('Meter Info', `${STRONPOWER_BASE}/api/Meter/info`, 'POST', {
        meter_no: 'TEST001'
    });
    await testEndpoint('Account Recharge', `${STRONPOWER_BASE}/api/Account/recharge`, 'POST', {
    ...
    await testEndpoint('Token API Health Check', `${TOKEN_METER_BASE}/health`);
    await testEndpoint('Token API - QueryMeterInfo', `${TOKEN_METER_BASE}/api/QueryMeterInfo`, 'POST', {
        "MeterNo": "TEST001"
    });
    await testEndpoint('Token API Root', TOKEN_METER_BASE);
    await testEndpoint('Token API — VendingMeter', `${TOKEN_METER_BASE}/api/VendingMeter`, 'POST', {
        "MeterNo": "TEST001",
        "Amount": 100
    });

    // Alternative common Rwanda token meter URLs
    await testEndpoint('Rwanda Token API (alt1)', 'https://token.rw/api/v1/recharge', 'POST', {
